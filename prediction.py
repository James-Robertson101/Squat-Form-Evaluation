...
def load_model_bundle(prefix, models_dir="models"):
    """Load the four artefacts needed for inference"""
    return {
...
def predict_reps(frame_folder, view="side", models_dir="models"):
    """
    Full pipeline: frames → features → predictions → feedback dicts.
    Returns a list of dicts, one per rep, ready for the feedback UI.
    """
    # Extract features
    per_rep_features = extract_squat_features_from_frames(frame_folder, view=view)
    ...
    # Get raw integer predictions — shape (n_reps, n_labels)
    raw_predictions = bundle["model"].predict(X_scaled)

    #  Map each integer directly to its feedback dict via FEEDBACK_MAP
